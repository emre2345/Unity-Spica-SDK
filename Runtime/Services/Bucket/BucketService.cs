...
namespace SpicaSDK.Services
{
    public partial class BucketService : ISpicaService
    {
        ...
        public async UniTask<Bucket[]> GetAllAsync()
        {
            var response = await httpClient.GetAsync(new Request(server.BucketUrl(new Id())));
        ...
        {
            var response =
                await httpClient.PostAsync(new Request(server.BucketUrl(new Id()),
                    JsonConvert.SerializeObject(bucket)));
        ...
        public class DataService : ISpicaService
        {
            ...
            {
                var response = await httpClient.PatchAsync(new Request(
                    server.BucketDataDocumentUrl(bucketId, documentId),
                    JsonConvert.SerializeObject(document),
                    new Dictionary<string, string>(0)));
            ...
            {
                var response = await httpClient.DeleteAsync(new Request(
                    server.BucketDataDocumentUrl(bucketId, documentId),
                    string.Empty,
                    new Dictionary<string, string>(0)));
